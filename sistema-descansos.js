(function () {
  ...
  function obterPontosDeVida(personagem) {
    return personagem?.combate?.pontosDeVida ?? personagem?.pontosDeVida ?? null;
  }
  ...
  function validarDescansoLongo(personagem, contexto = {}) {
    ...
    const tempoAtual = Number(contexto.tempoAtual ?? obterTempoAtual());

    const ultimoDescanso = contexto.ultimoDescansoLongoConcluidoEm ?? null;

    if (ultimoDescanso !== null && tempoAtual - ultimoDescanso < INTERVALO_ENTRE_DESCANSOS_LONGOS) {
      return {
        valido: false,
        motivo: "intervaloDescansoLongo",
        segundosRestantes: INTERVALO_ENTRE_DESCANSOS_LONGOS - (tempoAtual - ultimoDescanso),
      };
    }
  ...
  function prepararDescansoLongo(personagem, contexto = {}) {
    ...
    const vidaTemporariaAnterior = Number(pontosDeVida.temporarios ?? 0);

    pontosDeVida.atuais = vidaMaxima;
    pontosDeVida.temporarios = 0;

    const dadosVidaUsadosAnterior = Number(pontosDeVida.dadosVidaUsados ?? 0);

    pontosDeVida.dadosVidaUsados = 0;

    const recursosRecuperados = aplicarRecuperacaoDeRecursos(personagemAtualizado, "descansoLongo");
    ...
    return {
      ...
      recuperacao: {
        ...
        pontosDeVidaTemporariosRemovidos: vidaTemporariaAnterior,
        dadosVidaRecuperados: dadosVidaUsadosAnterior,
        recursos: recursosRecuperados,
      },
    };
  }

  function prepararRecuperacaoFimAventura(personagem) {
    if (!personagem) {
      return {
        sucesso: false,
        motivo: "personagemAusente",
    ...
    const recursos = personagemAtualizado.habilidades?.recursos ?? {};

    const recursosRecuperados = [];

    for (const [recursoId, recurso] of Object.entries(recursos)) {
      if (!recurso || typeof recurso !== "object") {
        continue;
      }

      const anterior = Number(recurso.usosAtuais ?? 0);

      const maximo = Number(recurso.usosMaximos ?? anterior);
      ...
      if (anterior !== maximo) {
        recursosRecuperados.push({
          ...
          anterior,
          atual: maximo,
          recuperado: maximo - anterior,
        });
      }
    }

    if (Array.isArray(personagemAtualizado.condicoes)) {
      personagemAtualizado.condicoes = [];
    }
  ...
  function recuperarPersonagemFimAventura() {
    ...
    const preparacao = prepararRecuperacaoFimAventura(personagem);
    ...
    if (window.estadoJogo?.personagem) {
      window.estadoJogo.personagem.condicoes = [];
    }
    ...
    return {
      ...
      recuperacao: preparacao.recuperacao,
    };
  }
  ...
  function realizarDescansoLongo() {
    ...
    const preparacao = prepararDescansoLongo(personagem, {
      tempoAtual,
      ultimoDescansoLongoConcluidoEm: registroDescansos.ultimoDescansoLongoConcluidoEm,
    });
    ...
    registroDescansos.ultimoDescansoLongoConcluidoEm = tempoConclusao;
  ...
  function iniciarDescansoCurto() {
    ...
    const recursosRecuperados = aplicarRecuperacaoDeRecursos(personagemAtualizado, "descansoCurto");
  ...
  function prepararGastoDadoVida() {
    ...
    const dadosVidaUsados = Number(pontosDeVida.dadosVidaUsados ?? 0);
    ...
    const constituicao = Number(personagem.atributos?.constituicao ?? 10);

    const modificadorConstituicao = calcularModificadorAtributo(constituicao);
  ...
  function aplicarResultadoDadoVida(resultadoRolagem) {
    ...
    const resultadoDado = Number(resultadoRolagem?.subtotal);
    ...
    const personagemAtualizado = copiar(obterPersonagemAtual());
    ...
    const curaCalculada = Math.max(0, resultadoDado + preparacao.modificadorConstituicao);

    const novaVida = Math.min(vidaMaxima, vidaAtual + curaCalculada);
    ...
    const resultado = {
      ...
      resultadoDado,
      modificadorConstituicao: preparacao.modificadorConstituicao,
      curaCalculada,
      curaAplicada,
      ...
      dadosVidaMaximos: Math.max(1, Number(personagemSalvo.nivel) || 1),
    };

    const registroDescansos = obterRegistroDescansos();

    registroDescansos.descansoCurtoAtual.dadosVidaGastos.push(resultado);
